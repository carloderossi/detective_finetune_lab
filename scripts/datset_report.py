...
def compute_qa_stats(df, tokenizer):
    ...
    empty_entries = df[df["text_stripped"].str.len() < 10]
...
def plot_clusters(df, embeddings, out_path):
    ...
    ax.set_title("t‑SNE – Style Clusters")
...
def plot_drift_heatmap(df, embedder, out_path, max_chapters=50, excerpt_len=512):
    holmes_df = df[df["author"] == "arthur_conan_doyle"].head(max_chapters).reset_index(drop=True)
    poirot_df = df[df["author"] == "agatha_christie"].head(max_chapters).reset_index(drop=True)

    if len(holmes_df) == 0 or len(poirot_df) == 0:
    ...
    holmes_labels = [f"H-{i+1}:{row['chapter']}" for i, (_, row) in enumerate(holmes_df.iterrows())]
    poirot_labels = [f"P-{i+1}:{row['chapter']}" for i, (_, row) in enumerate(poirot_df.iterrows())]
...
def write_markdown_report(df, stats, out_path):
    ...
    if avg_token_len > 4000:
        ...
        interp = "Entries are long; typical for chapter‑level data."
    else:
        interp = "Entries are relatively short; suitable for many fine‑tuning tasks."
    ...
    lines.append(f"- Number of empty entries: **{len(empty_entries)}**\n")
    if len(empty_entries) > 0:
        lines.append("| Author | Book | Chapter |")
        lines.append("|--------|------|---------|")
        for _, row in empty_entries.head(10).iterrows():
            lines.append(f"| {row['author']} | {row['book']} | {row['chapter']} |")
        lines.append("")

    lines.append("## Duplicate texts\n")
    lines.append(f"- Number of duplicate entries: **{len(dup_entries)}**\n")
    if len(dup_entries) > 0:
        ...
        for _, row in dup_entries.head(10).iterrows():
            ...
            if len(txt) > 80:
                txt = txt[:77] + "..."
            lines.append(f"| {row['author']} | {row['book']} | {row['chapter']} | {txt} |")
        lines.append("")
